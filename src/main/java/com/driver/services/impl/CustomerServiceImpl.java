...
bookedtrip.setBill(0);//bcoz we canceled the trip so set bill zero
bookedtrip.getDriver().getCab().setAvailable(true);//now this dirver available
...
//bookedtrip.getBill();
bookedtrip.getDriver().getCab().setAvailable(true);//after completing trip ,driver available for next
		tripBookingRepository2.save(bookedtrip);
	}
}
